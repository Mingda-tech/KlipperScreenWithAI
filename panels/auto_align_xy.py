...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        self._auto_align_timer = None
        self._seen_printing = False
        self._auto_align_checks = 0
        self.manual_step = "idle"
        self.auto_align_available = "KTAMV_AUTO_ALIGN" in self._printer.get_gcode_macros()
        ...
        offsetgrid = Gtk.Grid()
        self.labels['auto'] = self._gtk.Button(None, _("Auto"), "color1")
        self.labels['manual'] = self._gtk.Button(None, _("Manual"), "color1")

        self.labels['auto'].connect("clicked", self.auto_align)
        self.labels['manual'].connect("clicked", self.manual_align)
        offsetgrid.attach(self.labels['auto'], 0, 0, 1, 1)
        offsetgrid.attach(self.labels['manual'], 1, 0, 1, 1)
    ...
    def _reset_manual_state(self):
        self.manual_step = "idle"
        self.labels['manual'].set_label(_("Manual"))
        self.labels['manual'].set_sensitive(not self.auto_aligning)
        self.labels['auto'].set_sensitive(not self.auto_aligning and self.auto_align_available)

    def _set_action_buttons_busy(self):
        self.labels['auto'].set_sensitive(False)
        self.labels['manual'].set_sensitive(False)

    def _persist_offset_and_close(self):
        ...
        try:
            ...
            if self.pos['lx'] is not None and self.pos['ly'] is not None:
    ...
    def manual_align(self, widget):
        ...
        if self.manual_step == "idle":
            self.reset_pos()
            self.manual_step = "armed"
            self.labels['auto'].set_sensitive(False)
            self.labels['manual'].set_label(_("Confirm Pos"))
            return

        self.current_extruder = self._printer.get_stat("toolhead", "extruder")
        tool_number = self._printer.get_tool_number(self.current_extruder)

        if self.manual_step == "armed":
            if tool_number != 0:
                self._screen.show_popup_message(_("Please switch to the left extruder first."), level=2)
                return
            if self.pos['x'] is None or self.pos['y'] is None or self.pos['z'] is None:
                self._screen.show_popup_message(_("Unable to read the left extruder position."), level=2)
                return
            self.pos['lx'] = self.pos['x']
            self.pos['ly'] = self.pos['y']
            self.pos['lz'] = self.pos['z']
            self.manual_step = "t0_confirmed"
            self._screen.show_popup_message(
                _("Left extruder position: (%(x).2f, %(y).2f, %(z).2f)")
                % {"x": self.pos['lx'], "y": self.pos['ly'], "z": self.pos['lz']},
                level=1,
            )
        ...
        if self.manual_step == "t0_confirmed":
            if tool_number != 1:
                self._screen.show_popup_message(_("Please confirm the right extruder position."), level=2)
                return
            if self.pos['lx'] is None or self.pos['ly'] is None:
                self._screen.show_popup_message(_("Please confirm the left extruder position first."), level=2)
                return
            if self.pos['x'] is None or self.pos['y'] is None:
                self._screen.show_popup_message(_("Unable to read the right extruder position."), level=2)
                return
            self.pos['ox'] = self.pos['x'] - self.pos['lx']
            self.pos['oy'] = self.pos['y'] - self.pos['ly']
            self._screen.show_popup_message(
                _("Right extruder offset is (%(x).2f, %(y).2f)")
                % {"x": self.pos['ox'], "y": self.pos['oy']},
                level=1,
            )
            self._persist_offset_and_close()
    ...
    def back(self):
        if self.auto_aligning:
            self.auto_aligning = False
            if self._auto_align_timer:
                GLib.source_remove(self._auto_align_timer)
                self._auto_align_timer = None
        if self.camera_playing:
            self.stop_camera()
            self._run_light_off()
    ...
    def auto_align(self, widget):
        if self.auto_aligning:
            return
        if not self.auto_align_available:
            self._screen.show_popup_message(_("Auto-alignment is unavailable."), level=2)
            return
        state = self._printer.get_stat("idle_timeout", "state")
        if state == "Printing":
            self._screen.show_popup_message(_("Printer is busy."), level=2)
            return
        ...
        self._auto_align_checks = 0
        self.reset_pos()
        self._set_action_buttons_busy()

        self._screen.show_popup_message(_("Starting kTAMV auto-alignment..."), level=1)
        self._screen._ws.klippy.gcode_script("KTAMV_AUTO_ALIGN")
        self._auto_align_timer = GLib.timeout_add(3000, self._poll_auto_align)

    def _poll_auto_align(self):
        if not self.auto_aligning:
            return False

        self._auto_align_checks += 1

        state = self._printer.get_stat("idle_timeout", "state")
        if state == "Printing":
            self._seen_printing = True

        # Wait until macro finishes: must have seen Printing, then return to Ready/Idle
        if self._seen_printing and state in ("Ready", "Idle"):
            self._on_auto_align_complete()
            return False

        # Timeout after 5 minutes (100 * 3s)
        if self._auto_align_checks > 100:
            self.auto_aligning = False
            self._reset_manual_state()
            self._screen.show_popup_message(_("Auto-alignment timed out."), level=2)
            return False

        return True

    def _on_auto_align_complete(self):
        self.auto_aligning = False
        self._auto_align_timer = None
        self._reset_manual_state()

        # Re-read the variables file to get offset saved by the macro
        try:
            self._screen.klippy_config.read(self._screen.klippy_config_path)
            ox = self._screen.klippy_config.getfloat("Variables", "auto_align_x")
            oy = self._screen.klippy_config.getfloat("Variables", "auto_align_y")
            self.pos['ox'] = round(ox, 3)
            self.pos['oy'] = round(oy, 3)
            try:
                self.pos['lx'] = self._screen.klippy_config.getfloat("Variables", "cam_xpos")
                self.pos['ly'] = self._screen.klippy_config.getfloat("Variables", "cam_ypos")
            except Exception:
                pass
            self._screen.show_popup_message(
                _("Auto-alignment complete!\nOffset X: %(x).3f  Y: %(y).3f")
                % {"x": self.pos['ox'], "y": self.pos['oy']},
                level=1,
            )
            self._persist_offset_and_close()
        except Exception as e:
            logging.error(f"Failed to read auto-align results: {e}")
            self._screen.show_popup_message(_("Auto-alignment failed."), level=2)
    ...
    def activate(self):
        ...
        if not self.auto_align_available:
            self._screen.show_popup_message(_("Auto-alignment is unavailable."), level=1)
        self._run_light_on()

    def deactivate(self):
        # Stop auto-align polling
        if self.auto_aligning:
            self.auto_aligning = False
            if self._auto_align_timer:
                GLib.source_remove(self._auto_align_timer)
